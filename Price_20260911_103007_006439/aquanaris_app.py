...
[AQ1 · 2026-09-11] 루퍼젯 프로매니저(app.py V107)의 「🏪 아쿠나리스」 모드를 별도 앱으로 분리.
  ...
  · 공용(브랜드 UI·구글 연동·DB·로그인) = common/ · 아쿠나리스 전용 = aquanaris/ .
  · 🔴 looperget/(프로매니저 · 설계·견적·제안서)를 import 하지 않는다 — 시험으로 고정.
배포(같은 저장소 Looperget-Mate/Price · Streamlit 앱을 하나 더 만든다 · 메인 파일 = aquanaris_app.py):
  aquanaris_app.py + aquanaris_layout.py + aquanaris/ + common/  (+ looperget_brand.py · requirements.txt · .streamlit/ 는 저장소에 이미 있음)
실행: streamlit run aquanaris_app.py
"""
import os
...
try:
    ...
    import aquanaris as _aqn
    _VERS = (int(getattr(_cm, "COMMON_VER", 0) or 0), int(getattr(_aqn, "AQN_VER", 0) or 0))
except Exception:
    _VERS = (0, 0)
if _VERS[0] < 1 or _VERS[1] < 1:
    st.error("🚨 **`common/`·`aquanaris/` 폴더가 없거나 구버전입니다** — aquanaris_app.py(AQ1)와 짝이 맞지 않습니다.\n\n"
             "GitHub `Looperget-Mate/Price`에 **`common/`·`aquanaris/` 폴더를 통째로** "
             "`aquanaris_app.py`·`aquanaris_layout.py`와 함께 올린 뒤 재배포하세요.")
    st.stop()

from aquanaris.sheets import *
from aquanaris.layout import *   # [V66] 배치 엔진
if int(globals().get("AQ_LAYOUT_VER", 0) or 0) < 77:
    st.error("🚨 **aquanaris/layout.py가 구버전입니다** — aquanaris_app.py(AQ1)와 짝이 맞지 않습니다.")
    st.stop()

# [V72] 인쇄물(스티커·가이드북·배치도 PDF) — 시트·Drive·폰트 의존을 주입
from aquanaris import print_docs as _aqp
...
from aquanaris.print_docs import *
...
render_brand_footer("Aquanaris Builder")
