...
MASTER = os.path.join(ROOT, "_설계", "_농업제안서_마스터", "마스터_농업제안서_v1_20260824.pptx")
...
# ══════════════ 렌더러 ══════════════
class Renderer:
    def __init__(self, job: Dict, summary: Dict, out_pptx: str, work_dir: Optional[str] = None):
    ...
    def draw_lats(self, s, fr, w=W_LAT, ids=None):
        for l in self.lats:
            if ids is None or l["id"] in ids:
                ...
                pipe(s, fr, pts, color=ov.PIPE_SUB, weight=w)
    ...
    def build(self) -> str:
        ...
        # 01 표지
        replace_text(P["표지"], "{{대상지}}", "%s · %s" % (meta.get("site_short", S["site_label"]), S["name"]))
        ...
        caption(s, 8.7, 1.45, 4.3, S["name"], [
            ...
            "빨간 점선 = 이번 제안 대상지",
            tx.get("wide_note", ("회색 점선 = 기설 매설 배관 %.0f m (농가)" % self.water["buried_len_m"]) if self.water.get("buried") else "매설 배관 없음"),
            "초록 점 = %s" % self.water.get("start_kind", "급수 시작점"),
        ], title_color=ov.DIM)
        ...
        if not cap:
            ...
            for z in S["zones"]:
                cap.append("%s — 주배관 %.0f m · %d두" % (z["zone"], sum(r["len_m"] for r in self.design["mainline"]["routes"] if r["name"] in z["routes"]), z["n_heads"]))
            if S["n_joints"]:
            ...
            cap.append("꺾임은 모두 45° 이내 — 현장에서 호스를 굽힙니다(꺾임부 부속 없음)")
            if len(self.zone_color) > 1:
                names = {ov.PIPE_MAIN: "노랑", LINE2: "보라"}
                cap.append(" · ".join("%s = %s" % (names.get(c, "색"), z) for z, c in self.zone_color.items()))
            v = max((r["v50"] for r in S["hydro"]), default=0)
            if v > 2.0:
                cap.append("50 mm 유속 %.2f m/s(권장 2.0 초과) — 급수·정지는 밸브를 천천히(수격 방지)" % v)
        caption(s, CAP_X, CAP_Y, CAP_W, "주배관 50 mm · %d줄 · %d m" % (n_line, S["main_total_m"]), cap)
        ...
        cards.append(("c", "주배관 말단 마감 세트 — %d개소" % S["n_ends"],
                      "%s  E호스밸브 마감세트 50 (01403 + 호스밴드 2)" % S["sets"]["end50"],
                      [("02051", "송수호스", "50 mm"), ("00278", "호스밴드", "2¼″ ×2"), ("01403", "E호스밸브", "50 mm")],
                      "밸브로 막습니다 — 관 세척·월동 배수 때 밸브만 열면 주배관 안의 물이 빠집니다. 1개소 = E호스밸브 1 + 호스밴드 2."))
        if S["n_joints"]:
            cards.append(("b", "주배관 일자연결 세트 — %d개소" % S["n_joints"], S["sets"]["join50"],
                          [("00278", "호스밴드", "×2"), ("00825", "WF 4-1", "50"), ("00827", "WF 4-2", "50"), ("00278", "호스밴드", "×2")],
        ...
        if S["n_tees"] and meta.get("tee_panel", True):
            cards.append(("d", "주배관 T분기 세트 — %d개소" % S["n_tees"], S["sets"]["tee50"],
                          [("00278", "호스밴드", "×2"), ("00827", "WF 4-2", "50"), ("01201", "CCCT 中", "T"), ("00827", "WF 4-2", "50"), ("00278", "호스밴드", "×2")],
                          "한 줄이 여기서 양쪽으로 갈라집니다 — 되돌이(헤어핀) 배관을 만들지 않으려고 T를 씁니다. "
                          "1개소 = CCCT 中 1 + 나가는 쪽 WF 4-2 2 + 들어오는 쪽 WF 4-4 1 + 호스밴드 6 "
        ...
        if tx.get("start_on_prev"):
            ...
            rows.append(["a  시작부", S["sets"]["inlet50"], "1", "WF 4-10 1 + E호스밸브 1 + 호스밴드 2", "-"])
        if S["n_joints"]:
        ...
        if S["n_tees"]:
            rows.append(["d  T분기", S["sets"]["tee50"], str(S["n_tees"]),
                         "CCCT 中 1 + 나가는 쪽 WF 4-2 2 + 호스밴드 4 (들어오는 쪽 WF 4-4 1·밴드 2 = 매니폴드)",
        ...
        def spray_page(slide, title_new, ids, cap_title, cap_lines, only=None):
            ...
            for l in self.lats:
                if l["id"] in ids:
                    for hx, hy in l["heads"]:
                        spray_double(slide, fr, hx, hy, 10.0, 7.0)
        ...
        hyd = {r["zone"]: r for r in S["hydro"]}
        for k, z in enumerate(zs[:8]):
            h = hyd.get(z["zone"])
        ...
        replace_text(s, "{{합계금액}}", fmt_n(S["total"]))
        ...
        if vat0 and S["total"] < VAT_ZERO_MIN_TOTAL:
            self.log.append("영세율 안내 ON인데 합계 %s원 < 기준 %s원 — 대표 판단 확인 요"
                            % (fmt_n(S["total"]), fmt_n(VAT_ZERO_MIN_TOTAL)))
        elif not vat0 and S["total"] >= VAT_ZERO_MIN_TOTAL:
        ...
        steps = meta.get("next_steps") or [
            ...
            "시공 일정 — 자재 배송 뒤 현장 설치 지원 인력과 날짜를 잡습니다",
        ]
    ...
    def _bom_tables(self, slide, bom):
        ...
        for pi, part in enumerate(self._split_lines(self._bom_lines(bom))):
            ...
            for ri, row in enumerate(part, 1):
                ...
                put(t.cell(ri, 3), format(row["price"] or 0, ","), size=8, align=PP_ALIGN.RIGHT)
