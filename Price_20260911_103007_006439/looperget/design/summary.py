...
def build(job: Dict) -> Dict:
    ...
    money = design.get("money") or {"rows": [], "total": 0, "tier": "소비자가"}
    bom = []
    for r in money["rows"]:
    ...
    return {
        ...
        "cover": round(coverage(polys, heads), 3),
...
__all__ = ["build", "coverage", "edge_min", "joint_points", "sort_bom", "SETS", "PY", "SPRAY_R", "GROUP_ORDER", "GROUP_LABEL"]
