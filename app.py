...
# [핵심 수정] 드라이브의 모든 파일을 스캔하여 { '품목코드(파일명)': '파일ID' } 맵을 생성
# 캐싱 시간을 10분으로 설정하여 성능 최적화
@st.cache_data(ttl=600)
def get_drive_file_map():
    """드라이브 폴더 내의 모든 파일을 스캔하여 {파일명(확장자제외): 파일ID} 딕셔너리 반환"""
    folder_id = get_or_create_drive_folder()
    ...
    try:
        ...
        while True:
            ...
            for f in files:
                # 01733.jpg -> 01733 (확장자 제거한 이름을 키로 사용)
                name_stem = os.path.splitext(f['name'])[0]
                file_map[name_stem] = f['id'] # 값은 파일 ID
            
            page_token = response.get('nextPageToken', None)
            if page_token is None: break
    except Exception as e:
        # st.error(f"파일 목록 조회 중 오류: {e}")
        pass
        
    return file_map

def download_image_by_id(file_id):
    """파일 ID로 이미지를 다운로드하여 Base64 문자열 반환"""
...
def create_advanced_pdf(final_data_list, service_items, quote_name, quote_date, form_type, price_labels, buyer_info):
    # PDF 생성 시작 시, 드라이브 파일 목록을 최신화하여 가져옴 (중요)
    ...
    if st.session_state.quote_step == 1:
        ...
        if st.button("계산하기 (STEP 2)"):
            if not st.session_state.current_quote_name: st.error("현장명을 입력해주세요.")
                ...
                pipe_sums = {}
                for p_item in st.session_state.pipe_cart: pipe_sums[p_item['name']] = pipe_sums.get(p_item['name'], 0) + p_item['len']
