...
def init_db():
    ...
    # ── [V11] Config_JP 탭 분리 (KR/JP 동시 접속 시 구글 API 429 차단 방지) ──
    try: sh.worksheet("Config_JP")
    except:
        try:
            ws_cfg_jp = sh.add_worksheet(title="Config_JP", rows=10, cols=2)
            ws_cfg_jp.append_row(["항목", "비밀번호"])
            ws_cfg_jp.append_row(["app_pwd", "1234"])
            ws_cfg_jp.append_row(["admin_pwd", "1234"])
        except: pass
...
def load_data_from_sheet():
    ...
    data = {"config": {"app_pwd": "1234", "admin_pwd": "1234"}, "config_jp": {"app_pwd": "1234", "admin_pwd": "1234"}, "products": [], "sets": {}, "jp_quotes": [], "kr_quotes": []}
    ...
    # ── [V11] Config_JP 탭 분리 로드 ──
    try:
        sh = gc.open(SHEET_NAME)
        ws_config_jp = sh.worksheet("Config_JP")
        for rec in ws_config_jp.get_all_records():
            if rec.get("항목") == "app_pwd": data["config_jp"]["app_pwd"] = str(rec.get("비밀번호"))
            if rec.get("항목") == "admin_pwd": data["config_jp"]["admin_pwd"] = str(rec.get("비밀번호"))
    except: pass
...
def save_jp_quote_to_sheet(timestamp, q_name, manager, total, json_data):
    """JP 모드 견적 → Quotes_JP 탭에 별도 저장"""
    if not gc: return False
    try:
        sh = gc.open(SHEET_NAME)
        try: ws_jp = sh.worksheet("Quotes_JP")
        except: ws_jp = sh.add_worksheet(title="Quotes_JP", rows=100, cols=10); ws_jp.append_row(["날짜", "현장명", "담당자", "총액", "데이터JSON"])
...
if not st.session_state.app_authenticated:
    ...
    with col2:
        with st.container(border=True):
            # ── [V11] 로그인 전 KR/JP 선택 가능 (Config_JP 탭 분리)
            login_lang = st.radio("모드 선택", ["🇰🇷 한국용", "🇯🇵 日本用"], horizontal=True, key="login_lang_sel")
            is_jp_login = (login_lang == "🇯🇵 日本用")
            pwd = st.text_input("프로그램 접속 비밀번호", type="password", key="app_pwd")
            if st.button("접속 / アクセス", use_container_width=True):
                if is_jp_login:
                    app_pwd_db = str(st.session_state.db.get("config_jp", {}).get("app_pwd", "1234"))
                else:
                    app_pwd_db = str(st.session_state.db.get("config", {}).get("app_pwd", "1234"))
                if pwd == app_pwd_db:
                    st.session_state.app_authenticated = True
                    st.session_state.failed_attempts = 0
                    st.session_state.app_lang = "JP" if is_jp_login else "KR"
                    st.rerun()
                else:
...
if "target_set_edit" not in st.session_state: st.session_state.target_set_edit = None
...
if mode == "관리자 모드" or mode == "管理者モード":
    is_jp_admin = (st.session_state.app_lang == "JP" and mode == "管理者モード")
    
    if is_jp_admin:
        # ── [V11] JP 管理者モード: 읽기 전용 세트 조회 ──────────────
        st.header("🛠 管理者モード (照会専用)")
        st.info("🇯🇵 日本モードでは、セット情報の照会のみ可能です。追加・修正・削除は本社KRモードで行ってください。")
        if st.button("🔄 データ更新"): st.session_state.db = load_data_from_sheet(); st.success("完了"); st.rerun()
        
        # JP 管理者 비밀번호 - Config_JP 탭 사용
        if not st.session_state.auth_admin:
            pw = st.text_input("管理者パスワード", type="password")
            if st.button("ログイン"):
                jp_admin_pwd = str(st.session_state.db.get("config_jp", {}).get("admin_pwd", "1234"))
                if pw == jp_admin_pwd: st.session_state.auth_admin = True; st.rerun()
                else: st.error("パスワードが違います")
        else:
            if st.button("ログアウト"): st.session_state.auth_admin = False; st.rerun()
            st.subheader("📦 セット照会 (読み取り専用)")
            cat_jp_disp = st.selectbox("分類", ["주배관세트", "가지관세트", "기타자재"])
            cset_jp = st.session_state.db["sets"].get(cat_jp_disp, {})
            if cset_jp:
                for sname, sinfo in cset_jp.items():
                    with st.expander(f"📋 {sname} (部品数: {len(sinfo.get('recipe', {}))})"):
                        recipe = sinfo.get("recipe", {})
                        code_name_map_jp = {str(p.get("code","")): p.get("name","") for p in st.session_state.db["products"]}
                        if recipe:
                            rdf = pd.DataFrame([{"品目コード": k, "品目名": code_name_map_jp.get(str(k).zfill(5), k), "数量": v} for k,v in recipe.items()])
                            st.dataframe(rdf, hide_index=True, use_container_width=True)
                        else:
                            st.caption("レシピなし")
            else:
                st.info("登録されたセットがありません。")
        st.stop()
    ...
    if not st.session_state.auth_admin:
        ...
        with t3: 
            st.markdown("##### ⚙️ KR 비밀번호 설정 (Config 탭)")
            app_pwd_input = st.text_input("앱 접속 비밀번호 (KR)", value=st.session_state.db.get("config", {}).get("app_pwd", "1234"), key="cfg_app")
            admin_pwd_input = st.text_input("관리자/원가조회 비밀번호 (KR)", value=st.session_state.db.get("config", {}).get("admin_pwd", "1234"), key="cfg_admin")
            if st.button("💾 KR 비밀번호 변경 저장"):
                try:
                    sh = gc.open(SHEET_NAME)
                    ws_config = sh.worksheet("Config")
                    ws_config.clear()
                    ...
                    st.success("KR 비밀번호가 성공적으로 변경되었습니다!")
                except Exception as e:
                    st.error(f"비밀번호 저장 실패: {e}")

            st.divider()
            st.markdown("##### ⚙️ JP 비밀번호 설정 (Config_JP 탭)")
            st.caption("🇯🇵 일본 현지 앱 전용 비밀번호 — KR과 탭 분리로 API 429 차단 방지.")
            jp_app_pwd_input = st.text_input("앱 접속 비밀번호 (JP)", value=st.session_state.db.get("config_jp", {}).get("app_pwd", "1234"), key="cfg_jp_app")
            jp_admin_pwd_input = st.text_input("관리자 비밀번호 (JP)", value=st.session_state.db.get("config_jp", {}).get("admin_pwd", "1234"), key="cfg_jp_admin")
            if st.button("💾 JP 비밀번호 변경 저장"):
                try:
                    sh = gc.open(SHEET_NAME)
                    try: ws_cfg_jp = sh.worksheet("Config_JP")
                    except: ws_cfg_jp = sh.add_worksheet(title="Config_JP", rows=10, cols=2)
                    ws_cfg_jp.clear()
                    ws_cfg_jp.update([["항목", "비밀번호"], ["app_pwd", jp_app_pwd_input], ["admin_pwd", jp_admin_pwd_input]])
                    st.session_state.db.setdefault("config_jp", {})
                    st.session_state.db["config_jp"]["app_pwd"] = jp_app_pwd_input
                    st.session_state.db["config_jp"]["admin_pwd"] = jp_admin_pwd_input
                    st.success("JP 비밀번호가 성공적으로 변경되었습니다!")
                except Exception as e:
                    st.error(f"JP 비밀번호 저장 실패: {e}")
            ...
            if st.session_state.quote_step == 1:
                ...
                if st.session_state.final_edit_df is not None:
                    ...
                    if st.button("💾 見積保存 (Quotes_JPシート)"):
                        ...
                        if save_jp_quote_to_sheet(ts, st.session_state.current_quote_name, st.session_state.buyer_info.get("manager",""), int(total_jpy), json.dumps(jdata, ensure_ascii=False)):
