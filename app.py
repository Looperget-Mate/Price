...
import time
import xlsxwriter
from concurrent.futures import ThreadPoolExecutor  # [PATCH-A]
...
# ==========================================
st.set_page_config(layout="wide", page_title="루퍼젯 프로 매니저 V11.0")
...
# [PATCH-B] bytes 직접 업로드 함수 (캔버스 빌더 내보내기용)
def upload_bytes_to_drive(file_bytes: bytes, filename: str, mimetype: str = "image/png"):
    """BytesIO 없이 bytes를 직접 드라이브에 업로드. 파일명=세트명 규칙 유지."""
    folder_id = get_or_create_drive_folder()
    if not folder_id: return None
    try:
...
# ── [PATCH-B] 병렬 이미지 캐시 함수 ─────────────────────────────
def _fetch_one_set_image(args):
    """ThreadPoolExecutor용 단일 이미지 다운로드 워커"""
    set_name, img_ref = args
    return set_name, get_image_from_drive(img_ref)

def preload_set_images_parallel(sets_dict: dict) -> dict:
    """
    세트 딕셔너리에서 이미지 ID를 수집해 병렬로 한 번에 다운로드.
    결과는 st.session_state._img_cache에 저장.
    이미 캐시된 항목은 다운로드 생략.
    """
    ...
    for set_name, set_data in sets_dict.items():
        if not isinstance(set_data, dict):
            continue
        img_ref = set_data.get("image")
        if img_ref and set_name not in st.session_state._img_cache:
            tasks.append((set_name, img_ref))

    if not tasks:
        return st.session_state._img_cache

    with ThreadPoolExecutor(max_workers=8) as executor:
        results = list(executor.map(_fetch_one_set_image, tasks))

    for set_name, b64 in results:
        st.session_state._img_cache[set_name] = b64

    return st.session_state._img_cache

def get_cached_set_image(set_name: str, img_ref: str):
    """세션 캐시에서 이미지 반환. 없으면 단건 다운로드 후 캐싱."""
    if "_img_cache" not in st.session_state:
        st.session_state._img_cache = {}
...
def sync_products_jp_to_sheet(kr_products: list, exchange_rate: float):
    ...
    try:
        ...
        for i, p in enumerate(kr_products):
            ...
            if not code or code == "00000": continue
...
def create_advanced_pdf(final_data_list, service_items, quote_name, quote_date, form_type, price_labels, buyer_info, remarks):
    ...
    L = pdf.l_margin
    PAGE_W = 190
    LEFT_W = 95; RIGHT_W = 95; LBL_W = 24; VAL_W = LEFT_W - LBL_W; H_ROW = 7.0
    ...
    pdf.ln(3)
    if form_type == "basic":
        COL_IMG=25; COL_INFO=63; COL_UNIT=13; COL_QTY=13; COL_P1=29; COL_AMT=32; COL_RMK=15
    else:
        COL_IMG=25; COL_INFO=55; COL_UNIT=10; COL_QTY=10; COL_P1=18; COL_AMT1=22; COL_P2=18; COL_AMT2=22; COL_PROF=10
    def draw_table_header():
        pdf.set_fill_color(240, 240, 240)
        pdf.set_font(font_name, b_style, 9.5)
        H_HDR = 10
    ...
    draw_table_header()
    sum_qty=0; sum_a1=0; sum_a2=0; sum_profit=0; ITEM_H=18
    for item in final_data_list:
        ...
        if form_type == "profit":
            ...
            a2 = p2 * qty; sum_a2 += a2; profit = a2 - a1; sum_profit += profit
        ...
        if img_b64:
            try:
                ...
                pdf.image(tmp_path, x=x + (COL_IMG - img_sz) / 2, y=y + (ITEM_H - img_sz) / 2, w=img_sz, h=img_sz)
        ...
        if form_type == "basic":
            ...
            pdf.cell(COL_RMK, ITEM_H, "", border=1); pdf.ln()
        else:
            pdf.set_font(font_name, '', 8.5)
            pdf.cell(COL_P1, ITEM_H, f"{p1:,}", border=1, align='R')
            ...
            pdf.set_font(font_name, b_style, 8)
            pdf.cell(COL_PROF, ITEM_H, f"{rate:.1f}%", border=1, align='C'); pdf.ln()
    svc_total = 0
    if service_items:
        if pdf.get_y() + (len(service_items) * 7) + 10 > 265: pdf.add_page(); pdf.ln(1)
        else: pdf.ln(1)
    ...
    if pdf.get_y() + 12 > 265: pdf.add_page()
    ...
    if form_type == "basic":
        ...
        pdf.cell(COL_RMK, TOTAL_H, "", border=1, fill=True); pdf.ln()
    else:
        ...
        pdf.cell(COL_PROF, TOTAL_H, "", border=1, fill=True); pdf.ln()
    if remarks:
        pdf.ln(2)
...
def create_quote_excel(final_data_list, service_items, quote_name, quote_date, form_type, price_labels, buyer_info, remarks):
    ...
    f_val_11   = fmt(align='left', font_size=11, text_wrap=False)
    f_val_10   = fmt(align='left', font_size=10, text_wrap=False)
    ...
    if form_type == "basic":
        ...
        LAST_COL = 8
    for ci, cw in enumerate(col_widths): ws.set_column(ci, ci, cw)
    f_total_val_shrink = fmt(bold=True, bg_color='#E6E6E6', align='right', font_size=16, num_format='#,##0', shrink=True)
    ...
    left_rows  = [
        ("일련번호", serial or '/'), ("수  신", recipient or '/'), ("참  조", ref or '/'),
        ("TEL / FAX", tel_buyer or '/'), ("결재조건", pay_cond), ("유효기간", valid_per),
    ]
    right_rows = [
        ("사업자등록번호", "411-81-91898"), ("회사명/대표", "주식회사 신진켐텍 / 박형석"),
        ("주  소", "경기도 이천시 부발읍 황무로 1859-157"),
        ("업태/종목", "제조,도소매/산업용 밸브, 파이프 및 부속품 제조업"),
        ("담당자", manager), ("TEL/FAX", "031-638-1809 / 031-635-1801"),
    ]
    L_LBL=0; L_VAL=1; R_LBL_S=2; R_LBL_E=3; R_VAL_S=4; R_VAL_E=LAST_COL
    for i, ((ll, lv), (rl, rv)) in enumerate(zip(left_rows, right_rows)):
        r = i + 1; ws.set_row(r, 22)
        ws.write(r, L_LBL, ll, f_lbl); ws.write(r, L_VAL, lv, f_val_11)
        ws.merge_range(r, R_LBL_S, r, R_LBL_E, rl, f_lbl)
        rv_fmt = f_val_10 if i == 3 else f_val_11
        if R_VAL_S < R_VAL_E: ws.merge_range(r, R_VAL_S, r, R_VAL_E, rv, rv_fmt)
        else: ws.write(r, R_VAL_S, rv, rv_fmt)
    greeting = "1.귀사의 일의 번창을 기원합니다.\n2.하기와 같이 견적드리오니 검토하기 바랍니다."
    ws.merge_range(7, 0, 7, LAST_COL, greeting, f_greet); ws.set_row(7, 36.4)
    ws.set_row(8, 24)
    ws.write(8, COL_IMG, "이미지", f_hdr); ws.write(8, COL_INFO, "품목정보", f_hdr)
    ws.write(8, COL_UNIT, "단위", f_hdr); ws.write(8, COL_QTY, "수량", f_hdr)
    if form_type == "basic":
        ws.write(8, COL_P1, price_labels[0] if price_labels else "소비자가", f_hdr)
        ws.write(8, COL_AMT, "금액", f_hdr); ws.write(8, COL_RMK, "비고", f_hdr)
    else:
        l1 = price_labels[0] if price_labels else "단가1"
        l2 = price_labels[1] if len(price_labels) > 1 else "단가2"
        ws.write(8, COL_P1, l1, f_hdr); ws.write(8, COL_AMT1, "금액", f_hdr)
        ws.write(8, COL_P2, l2, f_hdr); ws.write(8, COL_AMT2, "금액", f_hdr)
        ws.write(8, COL_PROF, "이익율", f_hdr)
    ROW_H_ITEM=72; data_row=9; total_a1=0; total_a2=0; svc_total=0; temp_files=[]
    for item in final_data_list:
        ...
        if img_b64:
            try:
                img_data_str = img_b64.split(",", 1)[1] if "," in img_b64 else img_b64
                img_bytes = base64.b64decode(img_data_str)
                with Image.open(io.BytesIO(img_bytes)) as pil_img: orig_w, orig_h = pil_img.size
                with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                    tmp.write(img_bytes); tmp_path = tmp.name; temp_files.append(tmp_path)
                MARGIN=4; cell_w_px=int(14*7.5)-MARGIN*2; cell_h_px=int(ROW_H_ITEM*4/3)-MARGIN*2
                scale=min(cell_w_px/orig_w, cell_h_px/orig_h); fw=orig_w*scale; fh=orig_h*scale
                x_off=MARGIN+int((cell_w_px-fw)/2); y_off=MARGIN+int((cell_h_px-fh)/2)
                ws.insert_image(data_row, COL_IMG, tmp_path, {'x_scale':scale,'y_scale':scale,'x_offset':x_off,'y_offset':y_off,'object_position':2,'url':None})
            except: ws.write(data_row, COL_IMG, "No Img", f_img_cell)
        ...
        if form_type == "basic":
            ...
            a2=p2*qty; profit=a2-a1; rate=(profit/a2*100) if a2 else 0; total_a2+=a2
            ws.write(data_row, COL_QTY, qty, f_center_14_shrink)
            ws.write(data_row, COL_P1, p1, f_num_14_shrink)
    ...
    if service_items:
        ...
        ws.merge_range(data_row, 0, data_row, LAST_COL, "[ 추가 비용 ]", f_svc_hdr); data_row+=1
        for s in service_items:
            ws.set_row(data_row, 20)
            amt_col = COL_AMT if form_type=="basic" else COL_AMT2
            if amt_col>0: ws.merge_range(data_row, 0, data_row, amt_col-1, s['항목'], f_svc_val)
            else: ws.write(data_row, 0, s['항목'], f_svc_val)
            ws.write(data_row, amt_col, s['금액'], f_svc_num)
            for c in range(amt_col+1, NUM_COLS): ws.write(data_row, c, "", f_img_cell)
            svc_total+=s['금액']; data_row+=1
    ...
    for c in range(NUM_COLS): ws.write(data_row, c, "", fmt(border=0))
    data_row+=1
    if remarks:
        ws.set_row(data_row, 24)
        ws.merge_range(data_row, 0, data_row, LAST_COL, "특약사항 및 비고", f_rmk_hdr); data_row+=1
...
def create_composition_pdf(set_cart, pipe_cart, final_data_list, db_products, db_sets, quote_name):
    ...
    pdf.set_auto_page_break(False)
    pdf.add_page()
    has_font = os.path.exists(FONT_REGULAR); has_bold = os.path.exists(FONT_BOLD)
    font_name = 'NanumGothic' if has_font else 'Helvetica'; b_style = 'B' if has_bold else ''
    baseline_counts = {}; all_sets_db = {}
    ...
    additional_items_list = []; temp_baseline = baseline_counts.copy()
    for item in final_data_list:
        code = str(item.get("코드", "")).strip().zfill(5) if item.get("코드") else ""
        try: total_qty = int(float(item.get("수량", 0)))
        except: total_qty = 0
        name = item.get("품목", ""); spec = item.get("규격", ""); img_data = item.get("image_data", "")
        if code and code in temp_baseline:
            ...
            else: temp_baseline[code] -= total_qty
        else:
            if total_qty > 0:
                additional_items_list.append({"name": name, "spec": spec, "qty": total_qty, "code": code, "image": img_data})
    pdf.set_font(font_name, '', 10)
    pdf.cell(0, 8, f"현장명: {quote_name}", align='R', new_x="LMARGIN", new_y="NEXT"); pdf.ln(5)
    def check_page_break(h_needed):
        if pdf.get_y() + h_needed > 270: pdf.add_page()
    pdf.set_fill_color(220, 220, 220); pdf.set_font(font_name, b_style, 12)
    pdf.cell(0, 10, "1. 부속 세트 구성 (Fitting Sets)", border=1, fill=True, new_x="LMARGIN", new_y="NEXT")
    header_h=8; col_w_img=40; col_w_name=105; col_w_type=25; col_w_qty=20
    pdf.set_fill_color(240, 240, 240); pdf.set_font(font_name, b_style, 9)
    ...
    for item in set_cart:
        name=item.get('name'); qty=item.get('qty'); stype=item.get('type')
        recipe={}
        for cat, sets in db_sets.items():
            if name in sets: recipe=sets[name].get('recipe',{}); break
        recipe_lines=[]
        for p_code, p_qty in recipe.items():
            p_name=prod_code_to_name.get(str(p_code).strip().zfill(5), str(p_code))
            recipe_lines.append(f"  · {p_name}  ×{p_qty}")
        recipe_text="\n".join(recipe_lines)
        n_lines=max(len(recipe_lines), 1); row_h=max(5+n_lines*4.5+4, 22)
        check_page_break(row_h)
        img_id=None
        for cat, sets in db_sets.items():
            if name in sets: img_id=sets[name].get('image'); break
        img_b64=download_image_by_id(img_id)
        x, y=pdf.get_x(), pdf.get_y()
        pdf.cell(col_w_img, row_h, "", border=1)
        if img_b64:
            try:
                img_data=img_b64.split(",",1)[1]; img_bytes=base64.b64decode(img_data)
                with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                    tmp.write(img_bytes); tmp_path=tmp.name
                img_sz=min(col_w_img-6, row_h-5, 32)
                pdf.image(tmp_path, x=x+(col_w_img-img_sz)/2, y=y+(row_h-img_sz)/2, w=img_sz, h=img_sz)
                os.unlink(tmp_path)
            except: pass
        pdf.set_xy(x+col_w_img, y); pdf.cell(col_w_name, row_h, "", border=1)
        pdf.set_xy(x+col_w_img+2, y+2); pdf.set_font(font_name, b_style, 11)
        pdf.cell(col_w_name-4, 5.5, name, align='L')
        if recipe_text:
            pdf.set_xy(x+col_w_img+2, y+8); pdf.set_font(font_name, '', 8.5)
            pdf.multi_cell(col_w_name-4, 4.5, recipe_text, align='L', max_line_height=4.5)
        pdf.set_xy(x+col_w_img+col_w_name, y); pdf.set_font(font_name, '', 10)
    ...
    pdf.ln(5)
    pdf.set_font(font_name, b_style, 12); pdf.set_fill_color(220, 220, 220)
    check_page_break(20)
    ...
    for p in pipe_cart:
        ...
        if code not in pipe_summary: pipe_summary[code]={'len':0,'name':p.get('name'),'spec':p.get('spec')}
    ...
    for code, info in pipe_summary.items():
        ...
        img_id=get_best_image_id(code, img_val, drive_file_map); img_b64=download_image_by_id(img_id)
        x, y=pdf.get_x(), pdf.get_y()
        ...
        pdf.set_xy(x+22, y); pdf.set_font(font_name, '', 10)
    ...
    if additional_items_list:
        pdf.set_font(font_name, b_style, 12); pdf.set_fill_color(220, 220, 220); check_page_break(20)
        ...
        for item in additional_items_list:
            check_page_break(16)
            name=item['name']; spec=item['spec'] if item['spec'] else '-'; qty=item['qty']
            code=item.get('code'); img_val=item.get('image')
            img_id=get_best_image_id(code, img_val, drive_file_map); img_b64=download_image_by_id(img_id)
            x, y=pdf.get_x(), pdf.get_y()
            pdf.cell(22, 16, "", border=1)
            if img_b64:
    ...
    for item in final_data_list:
        ...
        if qty==0: continue
        check_page_break(16)
        name=item.get("품목",""); spec=item.get("규격","-"); code=item.get("코드",""); img_val=item.get("image_data")
        img_id=get_best_image_id(code, img_val, drive_file_map); img_b64=download_image_by_id(img_id)
        x, y=pdf.get_x(), pdf.get_y()
        pdf.cell(22, 16, "", border=1)
        if img_b64:
...
def create_composition_excel(set_cart, pipe_cart, final_data_list, db_products, db_sets, quote_name):
    ...
    def insert_scaled_image(ws, row, col, img_b64):
        if not img_b64: ws.write(row, col, "", fmt_center); return
        try:
            img_data=img_b64.split(",",1)[1] if "," in img_b64 else img_b64
            img_bytes=base64.b64decode(img_data)
            with Image.open(io.BytesIO(img_bytes)) as pil_img: orig_w, orig_h=pil_img.size; pil_img.close()
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                tmp.write(img_bytes); tmp_path=tmp.name; temp_files.append(tmp_path)
            cell_w_px=110; cell_h_px=106
            scale_x=cell_w_px/orig_w; scale_y=cell_h_px/orig_h; scale=min(scale_x, scale_y)*0.9
            final_w=orig_w*scale; final_h=orig_h*scale
            offset_x=(cell_w_px-final_w)/2; offset_y=(cell_h_px-final_h)/2
            ws.insert_image(row, col, tmp_path, {'x_scale':scale,'y_scale':scale,'x_offset':offset_x,'y_offset':offset_y,'object_position':1})
        except: ws.write(row, col, "Err", fmt_center)
    ws1=workbook.add_worksheet("부속세트")
    ws1.write(0,0,"이미지",fmt_header); ws1.write(0,1,"세트명",fmt_header)
    ws1.write(0,2,"구분",fmt_header); ws1.write(0,3,"수량",fmt_header)
    ws1.set_column(0,0,15); ws1.set_column(1,1,30)
    row=1
    for item in set_cart:
        ws1.set_row(row, 80); name=item.get('name'); img_id=None
        for cat, sets in db_sets.items():
            if name in sets: img_id=sets[name].get('image'); break
        insert_scaled_image(ws1, row, 0, download_image_by_id(img_id))
        ws1.write(row, 1, name, fmt_left); ws1.write(row, 2, item.get('type'), fmt_center)
        ws1.write(row, 3, item.get('qty'), fmt_center); row+=1
    ws2=workbook.add_worksheet("배관물량")
    ws2.write(0,0,"이미지",fmt_header); ws2.write(0,1,"품목명",fmt_header)
    ws2.write(0,2,"총길이(m)",fmt_header); ws2.write(0,3,"롤수",fmt_header)
    ws2.set_column(0,0,15); ws2.set_column(1,1,30)
    pipe_summary={}
    for p in pipe_cart:
        code=p.get('code')
        if not code: continue
    ...
    for code, info in pipe_summary.items():
        ...
        unit_len=prod_info.get("len_per_unit",4) if prod_info else 4
        if unit_len<=0: unit_len=4
        rolls=math.ceil(info['len']/unit_len); img_val=prod_info.get("image") if prod_info else None
        ...
        ws2.write(row, 2, info['len'], fmt_center); ws2.write(row, 3, rolls, fmt_center); row+=1
    if additional_items_list:
        ws_add=workbook.add_worksheet("추가자재")
        ws_add.write(0,0,"이미지",fmt_header); ws_add.write(0,1,"품목명",fmt_header)
        ws_add.write(0,2,"규격",fmt_header); ws_add.write(0,3,"추가수량",fmt_header)
        ws_add.set_column(0,0,15); ws_add.set_column(1,1,30)
        row=1
        for item in additional_items_list:
            ws_add.set_row(row, 80); img_val=item.get('image'); code=item.get('code')
            insert_scaled_image(ws_add, row, 0, download_image_by_id(get_best_image_id(code, img_val, drive_file_map)))
            ws_add.write(row, 1, item['name'], fmt_left); ws_add.write(row, 2, item['spec'], fmt_center)
            ws_add.write(row, 3, item['qty'], fmt_center); row+=1
    ws3=workbook.add_worksheet("전체자재")
    ws3.write(0,0,"이미지",fmt_header); ws3.write(0,1,"품목명",fmt_header)
    ws3.write(0,2,"규격",fmt_header); ws3.write(0,3,"총수량",fmt_header)
    ws3.set_column(0,0,15); ws3.set_column(1,1,30)
    row=1
    for item in final_data_list:
        try: qty=int(float(item.get("수량",0)))
        except: qty=0
        if qty==0: continue
        ws3.set_row(row, 80); code=item.get("코드",""); img_val=item.get("image_data")
        insert_scaled_image(ws3, row, 0, download_image_by_id(get_best_image_id(code, img_val, drive_file_map)))
        ws3.write(row, 1, item.get("품목",""), fmt_left); ws3.write(row, 2, item.get("규격","-"), fmt_center)
        ws3.write(row, 3, qty, fmt_center); row+=1
    workbook.close()
    for f in temp_files:
        try:
            if os.path.exists(f): os.unlink(f)
...
if "ui_state" not in st.session_state:
    st.session_state.ui_state = {"form_type":"기본 양식","print_mode":"개별 품목 나열 (기존)","vat_mode":"포함 (기본)","sel":["소비자가"]}
if "quote_remarks" not in st.session_state:
    st.session_state.quote_remarks = "1. 견적 유효기간: 견적일로부터 15일 이내\n2. 출고: 결재 완료 후 즉시 또는 7일 이내"
# [PATCH-B] 세션 이미지 캐시 초기화
if "_img_cache" not in st.session_state: st.session_state._img_cache = {}

# ── [PATCH-E] 세트 카드 툴팁 CSS 전역 선언 ───────────────────────
st.markdown("""
<style>
.set-card-wrap {
    position: relative;
...
if st.session_state.app_lang == "JP":
    if "jp_products_loaded" not in st.session_state or not st.session_state.get("jp_products_loaded"):
        st.session_state.db["jp_products"] = load_jp_merged_products(
            st.session_state.db["products"], st.session_state.exchange_rate)
...
with st.sidebar:
    ...
    if btn_save_temp or btn_save_off:
        ...
        if not q_name:
            ...
            ui_state_to_save = {"form_type":form_type_val,"print_mode":print_mode_val,"vat_mode":vat_mode_val,"sel":sel_val}
            save_data = {
                "items": st.session_state.quote_items, "services": st.session_state.services,
                "pipe_cart": st.session_state.pipe_cart, "set_cart": st.session_state.set_cart,
                "step": st.session_state.quote_step, "buyer": st.session_state.buyer_info,
                "remarks": st.session_state.quote_remarks, "custom_prices": current_custom_prices,
                "ui_state": ui_state_to_save, "save_type": save_type
            }
            est_total = 0
            ...
            for code, qty in st.session_state.quote_items.items():
                ...
                if prod: est_total += int(prod.get("price_cons", 0) or 0) * int(qty)
    ...
    if btn_init:
        ...
        st.session_state.quote_step = 1; st.session_state.current_quote_name = ""
        st.session_state.buyer_info = {"manager":"","phone":"","addr":"","serial":"","recipient":"","ref":"","pay_cond":"/","valid_period":"견적 후 15일 이내"}
        st.session_state.step3_ready = False; st.session_state.files_ready = False
        st.session_state.quote_remarks = "1. 견적 유효기간: 견적일로부터 15일 이내\n2. 출고: 결재 완료 후 즉시 또는 7일 이내"
        st.session_state.custom_prices = []
        st.session_state.ui_state = {"form_type":"기본 양식","print_mode":"개별 품목 나열 (기존)","vat_mode":"포함 (기본)","sel":["소비자가"]}
        st.session_state.last_sel = []
        st.session_state._img_cache = {}  # [PATCH] 이미지 캐시 초기화
    ...
    with col_lang1:
        ...
        if st.button("🇰🇷 한국용", use_container_width=True, type=kr_type, key="btn_lang_kr"):
            st.session_state.app_lang = "KR"; st.session_state.jp_products_loaded = False; st.rerun()
    with col_lang2:
        ...
        if st.button("🇯🇵 일본용", use_container_width=True, type=jp_type, key="btn_lang_jp"):
            st.session_state.app_lang = "JP"; st.session_state.jp_products_loaded = False; st.rerun()

    if st.session_state.app_lang == "JP":
        new_rate = st.number_input("환율 설정 (₩/¥)", value=st.session_state.exchange_rate, step=0.1, min_value=1.0, max_value=50.0, key="sidebar_exchange_rate")
        if new_rate != st.session_state.exchange_rate:
            st.session_state.exchange_rate = new_rate; st.session_state.jp_products_loaded = False; st.rerun()
    ...
    if kr_quotes:
        ...
        def format_quote_label(i):
            ...
            try: d_json = json.loads(d_json_str); s_type = d_json.get("save_type", "임시")
            except: s_type = "임시"
        ...
        if btn_load or btn_copy:
            try:
                ...
                st.session_state.ui_state = d.get("ui_state", {"form_type":"기본 양식","print_mode":"개별 품목 나열 (기존)","vat_mode":"포함 (기본)","sel":["소비자가"]})
        ...
        if btn_del:
            try:
                ...
                sh = gc.open(SHEET_NAME); ws_kr = sh.worksheet("Quotes_KR"); ws_kr.clear()
                if kr_quotes:
                    header = list(kr_quotes[0].keys())
                    rows = [header] + [[str(r.get(k,"")) for k in header] for r in kr_quotes]
                    ws_kr.update(rows)
                else: ws_kr.update([['날짜','현장명','담당자','총액','데이터JSON']])
                st.session_state.db = load_data_from_sheet()
...
# ── [PATCH-C] render_inputs_with_key — 병렬 캐시 + 호버 툴팁 ──────
def render_inputs_with_key(d, pf):
    """
    세트 선택 카드 렌더러.
    - 병렬 캐시: 최초 1회만 드라이브 API 호출, 이후 세션 캐시 사용
    - 호버 툴팁: 이미지 위쪽으로만 표시 → number_input과 겹치지 않음
    - 반환값: {세트명: 입력수량} dict (기존과 동일)
    """
    preload_set_images_parallel(d)
    prod_code_to_name = {
        str(p.get("code","")).strip().zfill(5): p.get("name","")
    ...
    for i, (n, v) in enumerate(d.items()):
        with cols[i % 4]:
            img_ref = v.get("image") if isinstance(v, dict) else None
            recipe = v.get("recipe", {}) if isinstance(v, dict) else {}
            if recipe:
                tooltip_lines = "<br>".join([
                    f"· {prod_code_to_name.get(str(k).zfill(5), str(k))} &nbsp;×{qty}"
                    for k, qty in recipe.items()
                ])
            else:
                tooltip_lines = "구성품 정보 없음"
            if img_ref:
                b64 = get_cached_set_image(n, img_ref)
                if b64:
                    st.markdown(f"""
<div class="set-card-wrap">
  <img src="{b64}" class="set-card-img"/>
  <div class="set-card-tooltip">{tooltip_lines}</div>
</div>""", unsafe_allow_html=True)
                else:
                    st.markdown("<div class='set-card-noimg'>No Image</div>", unsafe_allow_html=True)
            else:
                st.markdown("<div class='set-card-noimg'></div>", unsafe_allow_html=True)
            res[n] = st.number_input(n, 0, key=f"{pf}_{n}_input")
    return res

def render_inputs_jp(d, pf):
    """JP 모드용 — render_inputs_with_key와 동일한 캐시+툴팁 적용."""
    preload_set_images_parallel(d)
    prod_code_to_name = {
        str(p.get("code","")).strip().zfill(5): p.get("name","")
        for p in st.session_state.db.get("products", [])
...
if mode == "관리자 모드" or mode == "管理者モード":
    ...
    if st.button("🔄 구글시트 데이터 새로고침"):
        st.session_state.db = load_data_from_sheet()
        st.session_state._img_cache = {}  # [PATCH] 캐시 초기화
    ...
    if not st.session_state.auth_admin:
        ...
        with t1:
            ...
            with st.expander("📂 부품 데이터 직접 수정 (수정/추가/삭제)", expanded=True):
                ...
                edited_df = st.data_editor(
                    df, num_rows="dynamic", width="stretch", key="product_editor",
                    column_config={
                ...
                if st.session_state.get("confirming_product_save"):
                    ...
                    with col_no:
                        if st.button("❌ 아니오 (취소)"):
                            st.session_state.confirming_product_save = False; st.rerun()
            st.divider()
            ...
            with ec1:
                ...
                for eng_key in COL_MAP.values():
                    if eng_key not in org_df.columns:
                        org_df[eng_key] = 0 if ("price" in eng_key or "len" in eng_key) else ""
                    ...
                    with st.spinner("드라이브 폴더를 검색하는 중..."):
                        ...
                        if not file_map: st.warning("폴더가 비어있거나 찾을 수 없습니다.")
                            ...
                            if updated_count > 0:
                                ...
                                st.session_state.db = load_data_from_sheet()
                            else: st.warning("매칭되는 이미지가 없습니다. (파일명이 품목코드와 같은지 확인하세요)")
            st.divider()
            ...
            with st.expander("품목을 선택하여 매입단가 변경 시 다른 단가를 자동 재계산합니다.", expanded=False):
                ...
                recalc_target = st.selectbox(
                    "재계산 대상 품목 선택", products_for_recalc,
                ...
                if recalc_target:
                    ...
                    with col_new_buy:
                        new_buy_input = st.number_input("새 매입단가 (원)", min_value=0, value=old_buy, step=10, key="new_buy_input")
                    if new_buy_input > 0:
                        ...
                        with col_preview:
                            ...
                            for fk, label in KR_PRICE_LABELS.items():
                                ...
                                preview_rows.append({"_field": fk, "항목": label, "기존": fmt(old_v), "변경후": fmt(new_v)})
                            ...
                            for _, row in edited_preview.iterrows():
                                fk = row["_field"]
                                if fk in preview: preview[fk] = row["변경후"]
                            ...
                            if st.button("✅ 확인 — 단가 반영 및 저장", key="btn_recalc_confirm", type="primary"):
                                ...
                                for p in st.session_state.db["products"]:
                                    if str(p.get("code", "")).strip() == target_code:
                                        p.update(preview); p["last_updated"] = today_str
                                ...
                                st.success("✅ 한국 단가 저장 완료!"); st.rerun()
                        with col_cancel:
                            if st.button("❌ 취소", key="btn_recalc_cancel"): st.rerun()
                if st.session_state.get("pending_jp_sync"):
                    ...
                    with c_yes:
                        if st.button("🇯🇵 네, Products_JP 업데이트", type="primary", key="btn_jp_sync_yes"):
                            ...
                            st.session_state.pending_jp_sync = False; st.session_state.jp_products_loaded = False
                            if ok: st.success(f"✅ {msg}")
                            else: st.error(f"동기화 실패: {msg}")
                            st.rerun()
                    with c_no:
                        if st.button("나중에", key="btn_jp_sync_no"):
                            st.session_state.pending_jp_sync = False; st.rerun()
            st.divider()
        ...
        with t2:
            ...
            # ── [PATCH-D] 세트 이미지 빌더 ────────────────────────────────
            st.divider()
            with st.expander("🎨 세트 이미지 빌더 (캔버스에서 직접 제작 → 드라이브 저장)", expanded=False):
                st.caption("레시피가 등록된 세트의 이미지를 캔버스에서 직접 만들고 구글 드라이브에 저장합니다.\n기존 방식(외부 제작 후 드라이브 업로드 + 동기화)도 그대로 사용 가능합니다.")

                bcol1, bcol2 = st.columns(2)
                with bcol1:
                    builder_cat = st.selectbox("분류 선택", ["주배관세트", "가지관세트", "기타자재"], key="builder_cat")
                builder_sets = st.session_state.db["sets"].get(builder_cat, {})
                builder_set_names = list(builder_sets.keys())

                if not builder_set_names:
                    st.info("해당 분류에 등록된 세트가 없습니다.")
                else:
                    with bcol2:
                        builder_target = st.selectbox("이미지를 만들 세트 선택", builder_set_names, key="builder_target")
                    target_recipe = builder_sets.get(builder_target, {}).get("recipe", {})
                    ...
                    with rcol1:
                        ratio_label = st.selectbox("캔버스 비율", list(RATIO_OPTIONS.keys()), key="builder_ratio")
                    canvas_w, canvas_h = RATIO_OPTIONS[ratio_label]
                    bg_mode = st.radio("배경 모드", ["흰 배경 (정면도)", "아이소메트릭 격자 (등사각)"], horizontal=True, key="builder_bg")
                    st.markdown("**🖊️ 배관 표현 도구** (이미지에만 표시, 세트 데이터에 미포함)")
                    tcol1, tcol2, tcol3 = st.columns(3)
                    with tcol1:
                        pipe_tool = st.selectbox("배관 종류", ["없음", "🟡 호스 (노란색)", "⬛ 농수관 (짙은 회색)"], key="builder_pipe_tool")
                    with tcol2:
                        pipe_thickness = st.slider("배관 굵기", 4, 30, 10, key="builder_pipe_thick")
                    with tcol3:
                        pipe_opacity = st.slider("배관 투명도(%)", 30, 100, 80, key="builder_pipe_opacity")
                    pipe_color = "#FFD700" if "호스" in pipe_tool else "#555555"
                    ...
                    if target_recipe:
                        st.markdown("**📦 레시피 구성품** (참고용 — 캔버스에 자유롭게 배치하세요)")
                        pal_cols = st.columns(min(len(target_recipe), 5))
                        fmap_b = get_drive_file_map()
                        for idx, (code, qty) in enumerate(target_recipe.items()):
                            with pal_cols[idx % 5]:
                                p_name = prod_code_to_name_b.get(str(code).zfill(5), code)
                                img_id_b = fmap_b.get(str(code).zfill(5))
                                if img_id_b:
                                    b64_b = get_cached_set_image(f"_part_{code}", img_id_b)
                                    if b64_b: st.image(b64_b, use_container_width=True)
                                st.caption(f"{p_name}\n×{qty}")
                    else:
                        st.info("선택한 세트에 레시피가 없습니다. 먼저 구성품을 등록해주세요.")

                    iso_grid_js = ""
                    if "아이소메트릭" in bg_mode:
                        iso_grid_js = """
            ctx.save();
            ctx.strokeStyle='rgba(100,150,255,0.18)';ctx.lineWidth=1;
            var step=40;
            ...
            ctx.restore();"""
                    ...
                    canvas_html = f"""
<style>
#builderCanvas{{cursor:crosshair;border:2px solid #ddd;border-radius:6px;}}
#builderToolbar{{margin-bottom:8px;display:flex;gap:8px;flex-wrap:wrap;}}
#builderToolbar button{{padding:5px 12px;border-radius:4px;border:1px solid #ccc;cursor:pointer;font-size:13px;background:#f8f8f8;}}
#builderToolbar button.active{{background:#4a90d9;color:white;border-color:#4a90d9;}}
#builderStatus{{font-size:12px;color:#888;margin-top:4px;}}
</style>
<div id="builderToolbar">
  <button id="btnSelect" class="active" onclick="setMode('select')">🔲 선택</button>
  <button id="btnPipe" onclick="setMode('pipe')">🖊️ 배관 그리기</button>
  <button id="btnUndo" onclick="undo()">↩ 되돌리기</button>
  <button id="btnClear" onclick="clearCanvas()">🗑 전체 지우기</button>
  <span id="builderStatus">모드: 선택</span>
</div>
<canvas id="builderCanvas" width="{canvas_w}" height="{canvas_h}" style="max-width:100%;background:white;"></canvas>
<br>
<button onclick="exportCanvas()" style="margin-top:8px;padding:6px 18px;background:#27ae60;color:white;border:none;border-radius:5px;cursor:pointer;font-size:14px;">
  💾 이미지 내보내기 (PNG)
</button>
...
let mode='select',drawing=false,startX,startY;
let history=[],pipes=[];
let pipeColor='{pipe_color}',pipeWidth={pipe_thickness},pipeOpacity={pipe_opacity/100:.2f};
function setMode(m){{
    mode=m;
    document.querySelectorAll('#builderToolbar button').forEach(b=>b.classList.remove('active'));
    document.getElementById('btn'+m.charAt(0).toUpperCase()+m.slice(1)).classList.add('active');
    document.getElementById('builderStatus').textContent='모드: '+(m==='pipe'?'배관 그리기':'선택');
    canvas.style.cursor=m==='pipe'?'crosshair':'default';
}}
function redraw(){{
    ...
    {iso_grid_js}
    pipes.forEach(p=>{{
        ctx.save();ctx.globalAlpha=p.opacity;ctx.strokeStyle=p.color;
...
canvas.addEventListener('mousedown',e=>{{
    ...
    startX=(e.clientX-r.left)*sx;startY=(e.clientY-r.top)*sy;drawing=true;
}});
canvas.addEventListener('mouseup',e=>{{
    if(mode!=='pipe'||!drawing)return;
    const r=canvas.getBoundingClientRect();
    const sx=W/r.width,sy=H/r.height;
    const ex=(e.clientX-r.left)*sx,ey=(e.clientY-r.top)*sy;
    drawing=false;
    history.push(JSON.parse(JSON.stringify(pipes)));
    pipes.push({{x1:startX,y1:startY,x2:ex,y2:ey,color:pipeColor,width:pipeWidth,opacity:pipeOpacity}});
    redraw();
}});
function undo(){{if(history.length>0){{pipes=history.pop();redraw();}}}}
function clearCanvas(){{history.push(JSON.parse(JSON.stringify(pipes)));pipes=[];redraw();}}
function exportCanvas(){{
    const link=document.getElementById('downloadLink');
    link.href=canvas.toDataURL('image/png');
    link.download='{builder_target}.png';
    link.click();
}}
</script>"""
                    st.components.v1.html(canvas_html, height=canvas_h + 130, scrolling=False)

                    st.markdown("---")
                    st.markdown("**📤 캔버스에서 내보낸 PNG를 드라이브에 직접 저장**")
                    st.caption(f"파일명은 자동으로 세트명 **'{builder_target}'** 으로 저장됩니다. (기존 이미지는 덮어쓰기)")
                    builder_upload = st.file_uploader("내보낸 PNG 파일 선택", type=["png","jpg","jpeg"], key=f"builder_upload_{builder_target}")
                    if builder_upload:
                        file_ext = builder_upload.name.split('.')[-1].lower()
                        save_filename = f"{builder_target}.{file_ext}"
                        if st.button(f"☁️ 구글 드라이브에 '{save_filename}' 저장", key="btn_builder_save", type="primary"):
                            with st.spinner("드라이브에 업로드 중..."):
                                file_bytes = builder_upload.getvalue()
                                mimetype = "image/png" if file_ext=="png" else "image/jpeg"
                                new_img_id = upload_bytes_to_drive(file_bytes, save_filename, mimetype)
                            if new_img_id:
                                st.session_state.db["sets"][builder_cat][builder_target]["image"] = new_img_id
                                save_sets_to_sheet(st.session_state.db["sets"])
                                # [PATCH] 해당 세트 캐시 개별 무효화
                                st.session_state._img_cache.pop(builder_target, None)
                                get_drive_file_map.clear()
                                st.success(f"✅ '{save_filename}' 이 드라이브에 저장되고 세트 이미지로 등록되었습니다!")
                                st.rerun()
                            else:
                                st.error("드라이브 저장 실패. 아래 [이미지 업로드/변경] 또는 수동 드라이브 업로드+동기화를 사용해주세요.")
            # ── [PATCH-D] 세트 이미지 빌더 끝 ────────────────────────────
                    ...
                    if len(sel_rows) == 1:
                        ...
                        with col_edit:
                            if st.button(f"✏️ '{tg}' 구성품 수정하기", use_container_width=True):
                                ...
                                st.session_state.set_manage_mode = "수정"; st.rerun()
                        with col_img:
                            with st.expander("🖼️ 세트 이미지 관리", expanded=True):
                                ...
                                if current_img_id:
                                    ...
                                    if st.button("🗑️ 이미지 삭제", key=f"del_img_{tg}"):
                                        ...
                                        save_sets_to_sheet(st.session_state.db["sets"])
                                        st.session_state._img_cache.pop(tg, None)  # [PATCH] 캐시 무효화
                                        st.success("이미지가 삭제되었습니다."); st.rerun()
                                else:
                                ...
                                if set_img_file:
                                    if st.button("💾 이미지 저장", key=f"save_img_{tg}"):
                                        with st.spinner("이미지 업로드 중..."):
                                            ...
                                            new_filename = f"{tg}.{file_ext}"  # 파일명=세트명 규칙 적용
                            ...
                            if del_pw == admin_pwd_db:
                                ...
                                for name in target_names:
                                    if name in st.session_state.db["sets"][cat]:
                                        ...
                                        st.session_state._img_cache.pop(name, None)  # [PATCH]
                                        del_count += 1
                                save_sets_to_sheet(st.session_state.db["sets"])
            ...
            with st.expander("📂 드라이브에 올린 파일과 세트 자동 연결하기", expanded=False):
                ...
                if st.button("🔄 드라이브 세트 이미지 자동 동기화", key="btn_sync_set_images"):
                    with st.spinner("드라이브 폴더를 검색하는 중..."):
                        get_drive_file_map.clear()
                        file_map = get_drive_file_map()
                        if not file_map: st.warning("폴더를 찾을 수 없거나 비어있습니다.")
                        else:
                            updated_count = 0
                            ...
                            for cat_key, cat_items in all_sets.items():
                                for s_name, s_data in cat_items.items():
                                    if s_name in file_map:
                                        s_data["image"] = file_map[s_name]; updated_count += 1
                                    elif f"{s_name}_image" in file_map:
                                        s_data["image"] = file_map[f"{s_name}_image"]; updated_count += 1
                            if updated_count > 0:
                                save_sets_to_sheet(all_sets)
                                st.session_state._img_cache = {}  # [PATCH] 전체 캐시 초기화
            ...
            if mt == "신규":
                ...
                if st.session_state.temp_set_recipe:
                    ...
                    edited_recipe = st.data_editor(
                        pd.DataFrame(recipe_list), num_rows="dynamic", width="stretch", hide_index=True,
                        disabled=["품목코드", "품목명"],
                        column_config={"삭제": st.column_config.CheckboxColumn(label="삭제?", default=False)},
                ...
                if "target_set_edit" in st.session_state and st.session_state.target_set_edit:
                    ...
                    with c3:
                        ...
                        if st.button("담기", key="esa"):
                            st.session_state.temp_set_recipe[str(ap_obj['code'])] = aq; st.rerun()
                    ...
                    if st.button(f"🗑️ '{tg}' 세트 영구 삭제", key="btn_del_set"):
                        ...
                        save_sets_to_sheet(st.session_state.db["sets"])
                        st.session_state._img_cache.pop(tg, None)  # [PATCH]
        ...
        with t3:
            ...
            if st.button("💾 비밀번호 변경 저장"):
                try:
                    sh = gc.open(SHEET_NAME); ws_config = sh.worksheet("Config"); ws_config.clear()
    ...
    if st.button("🔄 데이터 새로고침"):
        st.session_state.db = load_data_from_sheet(); st.rerun()
    ...
    if not jp_quotes:
        ...
        selected_quote_idx = st.selectbox(
            "분석 대상 견적 선택", range(len(df_quotes)),
        ...
        except: items_dict = {}; st.error("JSON 데이터 파싱 실패")
        if items_dict:
            pdb_map = {str(p.get("code")).strip().zfill(5): p for p in st.session_state.db["products"]}
            analysis_data = []
            for code, qty in items_dict.items():
                clean_code = str(code).strip().zfill(5); qty = int(qty)
                prod = pdb_map.get(clean_code)
                if prod:
                    p_buy = int(prod.get("price_buy", 0)); p_supply = int(prod.get("price_supply_jp", 0))
                    total_rev = p_supply * qty; total_cost = p_buy * qty; profit = total_rev - total_cost
                    analysis_data.append({"품목코드":clean_code,"품목명":prod.get("name",""),"규격":prod.get("spec","-"),"수량":qty,"매입단가(원)":p_buy,"신정공급가(원)":p_supply,"합계매출":total_rev,"합계원가":total_cost,"순이익":profit})
                else:
                    analysis_data.append({"품목코드":clean_code,"품목명":"미등록 품목","규격":"-","수량":qty,"매입단가(원)":0,"신정공급가(원)":0,"합계매출":0,"합계원가":0,"순이익":0})
            ...
            t_rev = df_analysis["합계매출"].sum(); t_cost = df_analysis["합계원가"].sum()
            t_profit = df_analysis["순이익"].sum(); margin = (t_profit / t_rev * 100) if t_rev > 0 else 0
            st.divider()
            ...
            if st.button("📄 수출 이익 분석서 생성"):
                with st.spinner("보고서를 생성하고 있습니다..."):
                    ...
                    cols_h = ["Code","Item Name","Spec","Qty","Buy Price","Supply Price","Sum Revenue","Sum Cost","Profit"]
                    widths = [20, 50, 40, 15, 30, 30, 35, 35, 30]
                    for head, w in zip(cols_h, widths): pdf.cell(w, 10, head, border=1, align='C', fill=True)
                    pdf.ln()
        ...
        if not jp_products:
            ...
            if st.session_state.quote_step == 1:
                ...
                with st.expander("セット選択", True):
                    ...
                    if st.button("➕ セットリストに追加"):
                        ...
                        for k, v in all_inp.items():
                            if v > 0: st.session_state.set_cart.append({"name":k,"qty":v,"type":"メイン管"})
                        st.rerun()
                with st.expander("配管数量入力"):
                    ptype = st.radio("配管区分", ["주배관","가지관"], horizontal=True, key="jp_pipe_radio", format_func=lambda x: "メイン配管" if x=="주배관" else "分岐配管")
                ...
                if st.session_state.final_edit_df is not None:
                    edited_jp = st.data_editor(
                        st.session_state.final_edit_df[["品目","規格","コード","単位","数量","price_1"]],
                        num_rows="dynamic", hide_index=True,
                        column_config={"price_1": st.column_config.NumberColumn("消費者価格(¥)", format="%d")},
                        use_container_width=True, key="jp_final_editor"
                    )
                ...
                with c2:
                    if st.button("🔄 最初から"):
                        ...
                        st.session_state.pipe_cart=[]; st.session_state.set_cart=[]; st.session_state.current_quote_name=""; st.rerun()
        st.stop()
    ...
    if st.session_state.quote_step == 1:
        ...
        with st.expander("👤 구매자(현장) 정보 입력", expanded=True):
            ...
            st.session_state.buyer_info.update({"manager":manager,"phone":phone,"addr":addr,"recipient":recipient,"ref":ref,"pay_cond":pay_cond,"valid_period":valid_period})
        st.divider()
        sets = st.session_state.db.get("sets", {})
            ...
            if st.button("➕ 입력한 수량 세트 목록에 추가"):
                ...
                for set_name, qty in all_inputs.items():
                    if qty > 0:
                        ...
                        added_count += 1
                if added_count > 0: st.success(f"{added_count}개 항목이 목록에 추가되었습니다.")
        ...
        with st.expander("2. 가지관 및 기타 세트"):
            ...
            if st.button("➕ 가지관/기타 목록 추가"):
                all_inputs = {**inp_b, **inp_e}; added_count = 0
                for set_name, qty in all_inputs.items():
                    if qty > 0:
        ...
        if st.session_state.set_cart:
            ...
            cart_df = pd.DataFrame(st.session_state.set_cart); cart_df["삭제"] = False
            edited_cart = st.data_editor(
                cart_df, width="stretch", hide_index=True, disabled=["name", "type"],
                column_config={
            ...
            with c_btn1:
                if st.button("💾 세트 목록 변경사항 적용", use_container_width=True):
                    ...
                    for _, row in edited_cart.iterrows():
                        if not row.get("삭제"): new_cart.append({"name":row["name"],"qty":int(row["qty"]),"type":row["type"]})
                    st.session_state.set_cart = new_cart; st.rerun()
            with c_btn2:
                if st.button("🗑️ 세트 목록 전체 비우기", use_container_width=True):
                    st.session_state.set_cart = []; st.rerun()
        st.divider()
        ...
        if st.button("계산하기 (STEP 2)"):
            if not st.session_state.current_quote_name: st.error("현장명을 입력해주세요.")
                ...
                st.session_state.quote_items = res; st.session_state.quote_step = 2
                st.session_state.step3_ready = False; st.session_state.files_ready = False; st.rerun()
        ...
        for n, q in st.session_state.quote_items.items():
            inf = pdb.get(str(n), {})
            if not inf: continue
            if view == "소비자가" and inf.get("category", "") == "관급비용": continue
        ...
        df = pd.DataFrame(rows) if rows else pd.DataFrame(columns=disp)
        ...
        with st.expander("🛒 추가된 부품 수정 및 삭제", expanded=False):
            ...
            for k, v in st.session_state.quote_items.items():
                inf = pdb.get(str(k), {})
                p_code = inf.get("code", str(k)); p_name = inf.get("name", str(k))
                parts_list.append({"품목코드":p_code,"품목명":p_name,"수량":int(v),"삭제":False,"_orig_key":str(k)})
            if parts_list:
                parts_df = pd.DataFrame(parts_list)
                edited_parts = st.data_editor(
                    parts_df, width="stretch", hide_index=True, disabled=["품목코드","품목명"],
                    column_config={"삭제":st.column_config.CheckboxColumn("삭제?",default=False),"수량":st.column_config.NumberColumn("수량",min_value=1,step=1),"_orig_key":None},
                    key="parts_cart_editor"
                )
                if st.button("💾 부품 변경사항 적용", use_container_width=True):
                    new_quote_items = {}
                    for _, row in edited_parts.iterrows():
                        if not row.get("삭제"): new_quote_items[row["_orig_key"]] = int(row["수량"])
                    st.session_state.quote_items = new_quote_items; st.rerun()
            else: st.info("장바구니에 담긴 부품이 없습니다.")
        st.divider()
            ...
            with st.container(border=True):
                ...
                with c_btn:
                    ...
                    if st.button("추가", use_container_width=True):
                        st.session_state.quote_items[str(ap_obj['code'])] = st.session_state.quote_items.get(str(ap_obj['code']), 0) + aq; st.rerun()
        with col_add_cost:
            st.markdown("##### 💰 비용 추가")
            with st.container(border=True):
        ...
        if st.button("최종 확정 (STEP 3)", type="primary", use_container_width=True):
            st.session_state.quote_step = 3; st.session_state.step3_ready = False; st.session_state.files_ready = False; st.rerun()
        ...
        pkey = {"매입단가":"price_buy","총판가1":"price_d1","총판가2":"price_d2","대리점가1":"price_agy1","대리점가2":"price_agy2","계통농협":"price_nh_sys","지역농협":"price_nh_loc","소비자가":"price_cons","단가(현장)":"price_site"}
        ...
        if not st.session_state.step3_ready or selectors_changed:
            ...
            fdata = []; processed_keys = set()
            for n, q in st.session_state.quote_items.items():
                inf = pdb.get(str(n), {})
                if not inf: continue
                if "소비자가" in sel and inf.get("category","") == "관급비용": continue
                ...
                d = {"품목":name_val,"규격":inf.get("spec",""),"코드":inf.get("code",""),"단위":inf.get("unit","EA"),"수량":int(q),"image_data":inf.get("image")}
        ...
        def on_data_change(): st.session_state.files_ready = False
        with st.expander("➕ 수기 품목 추가 (DB 미등록 품목)", expanded=False):
            c1, c2, c3, c4, c5 = st.columns([3,2,1,1,2])
            m_name=c1.text_input("품목명 (필수)", key="m_name"); m_spec=c2.text_input("규격", key="m_spec")
            m_unit=c3.text_input("단위","EA",key="m_unit"); m_qty=c4.number_input("수량",1,key="m_qty")
            m_price=c5.number_input("단가",0,key="m_price")
            if st.button("리스트에 추가", key="btn_add_manual"):
                if m_name:
                    new_row = {"품목":m_name,"규격":m_spec,"코드":"","단위":m_unit,"수량":m_qty,"price_1":m_price,"price_2":0,"image_data":""}
                    st.session_state.final_edit_df = pd.concat([st.session_state.final_edit_df, pd.DataFrame([new_row])], ignore_index=True)
                    st.session_state.files_ready = False; st.rerun()
                else: st.warning("품목명을 입력해주세요.")
        edited = st.data_editor(
            st.session_state.final_edit_df[disp_cols], num_rows="dynamic", width="stretch", hide_index=True,
            column_config={
            ...
            if st.button("📄 견적서 파일 생성하기 (PDF/Excel)", type="primary", use_container_width=True):
                with st.spinner("파일을 생성하고 있습니다... (이미지 다운로드 및 변환 중)"):
                    ...
                    if print_mode == "세트별 부품 분해 (납품 패킹용)":
                        expanded_data = []; pool = {}; price_map_1 = {}; price_map_2 = {}
                        for item in safe_data:
                        ...
                        for s_item in st.session_state.set_cart:
                            ...
                            for p_code_or_name, p_qty_per_set in recipe.items():
                                ...
                                expanded_data.append({"품목":f"[{s_name}] {prod_info.get('name',p_key)}","규격":prod_info.get("spec",""),"코드":prod_info.get("code",p_key),"단위":prod_info.get("unit","EA"),"수량":req_qty,"price_1":price_map_1.get(p_key,0),"price_2":price_map_2.get(p_key,0),"image_data":prod_info.get("image","")})
                                if p_key in pool: pool[p_key]-=req_qty
                        for p_item in st.session_state.pipe_cart:
                            p_code=p_item.get('code'); p_len=p_item.get('len',0)
                            prod_info=next((p for p in st.session_state.db["products"] if str(p.get("code","")).strip().zfill(5)==p_code), {})
                            unit_len=prod_info.get("len_per_unit",4) if prod_info else 4
                            req_qty=math.ceil(p_len/(unit_len if unit_len>0 else 4)); p_key=str(p_code).strip().zfill(5)
                            expanded_data.append({"품목":f"[배관] {prod_info.get('name',p_item.get('name'))}","규격":prod_info.get("spec",p_item.get("spec","")),"코드":p_code,"단위":prod_info.get("unit","EA"),"수량":req_qty,"price_1":price_map_1.get(p_key,0),"price_2":price_map_2.get(p_key,0),"image_data":prod_info.get("image","")})
                            if p_key in pool: pool[p_key]-=req_qty
                        for item in safe_data:
                            k=str(item.get("코드","")).strip().zfill(5)
                            if k=="00000" or not k: k=str(item.get("품목","")).strip()
                            ...
                            if rem_qty>0:
                                new_item=item.copy(); new_item["품목"]=f"[추가/별도] {item.get('품목')}"; new_item["수량"]=rem_qty
                                expanded_data.append(new_item); pool[k]=0
                        ...
                        for s_item in st.session_state.set_cart:
                            s_name=s_item['name']; s_qty=s_item['qty']
                            if s_qty<=0: continue
                            s_price1=0; s_price2=0; s_img=""
                            if s_name in all_sets_db:
                                recipe=all_sets_db[s_name].get("recipe",{}); s_img=all_sets_db[s_name].get("image","")
                                for p_code_or_name, p_qty_per_set in recipe.items():
                                    p_key=str(p_code_or_name).strip().zfill(5)
                                    if p_key not in comp_pool: p_key=str(p_code_or_name).strip()
                                    s_price1+=(comp_price1.get(p_key,0)*p_qty_per_set)
                                    s_price2+=(comp_price2.get(p_key,0)*p_qty_per_set)
                                    if p_key in comp_pool: comp_pool[p_key]-=(p_qty_per_set*s_qty)
                            set_items_out.append({"품목":s_name,"규격":"세트","코드":s_name,"단위":"SET","수량":s_qty,"price_1":s_price1,"price_2":s_price2,"image_data":s_img})
                        rem_items_out=[]
                        for item in safe_data:
                            match_key=str(item.get("코드","")).strip().zfill(5)
                            if not match_key or match_key=="00000": match_key=str(item.get("품목","")).strip()
        ...
        with c1:
            if st.button("⬅️ 수정 (이전 단계)"):
                st.session_state.quote_step = 2; st.session_state.step3_ready = False; st.session_state.files_ready = False; st.rerun()
        with c2:
            if st.button("🔄 처음으로"):
                st.session_state.quote_step = 1; st.session_state.quote_items = {}; st.session_state.services = []
                st.session_state.pipe_cart = []; st.session_state.set_cart = []
                st.session_state.buyer_info = {"manager":"","phone":"","addr":"","serial":"","recipient":"","ref":"","pay_cond":"/","valid_period":"견적 후 15일 이내"}
                st.session_state.current_quote_name = ""; st.session_state.step3_ready = False; st.session_state.files_ready = False; st.rerun()
