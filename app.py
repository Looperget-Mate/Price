...
def get_image_from_drive(filename):
    ...
    try:
        ...
        # [원복 및 수정] 메모리 버퍼(io.BytesIO) 사용으로 무한 로딩 방지
        fh = io.BytesIO()
...
def create_advanced_pdf(final_data_list, service_items, quote_name, quote_date, form_type, price_labels, recipient_info):
    try:
        ...
        pdf.set_xy(10, top_y); pdf.set_font(font_name, '', 10); pdf.cell(90, 8, " [ Customer Info ]", border=0, ln=1)
        ...
        pdf.set_xy(105, top_y); pdf.set_font(font_name, '', 10); pdf.cell(90, 8, " [ Supplier Info ]", border=0, ln=1)
        ...
        for item in final_data_list:
            ...
            if img_b64:
                try:
                    # Base64 문자열을 디코딩하여 temp 파일로 저장 후 FPDF에 전달
                    ...
                    os.unlink(tmp_img_path) # 사용 후 삭제
                except: pass
        ...
        # [수정] PDF 출력 (Latin-1 인코딩 - 다운로드 에러 해결)
...
if mode == "관리자 모드":
    ...
    if not st.session_state.auth_admin:
        ...
        with t1:
            ...
            with st.expander("📂 엑셀 데이터 등록/다운로드 (클릭)", expanded=True):
                ...
                ordered_cols = ["order_no", "code", "image", "category", "name", "spec", "unit", "len_per_unit", "price_buy", "price_d1", "price_d2", "price_agy", "price_cons", "price_site"]
                disp_cols = []
    ...
    if st.session_state.quote_step == 1:
        ...
        rows = []; pdb_by_code = {p["code"]: p for p in st.session_state.db["products"]}
        
        for code, q in st.session_state.quote_items.items():
            inf = pdb_by_code.get(code, {})
            if not inf: continue
            
            n = inf['name']; cpr = inf.get("price_cons", 0)
