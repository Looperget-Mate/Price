...
def upload_bytes_to_drive(file_bytes: bytes, filename: str, mimetype: str = "image/png") -> str | None:
    """
    [PATCH-D 전용] bytes 데이터를 구글드라이브 Looperget_Images 폴더에 직접 업로드.
    반환값: 드라이브 file ID (세트 데이터의 'image' 필드에 저장)
    """
    folder_id = get_or_create_drive_folder()
    if not folder_id or not drive_service: return None
    try:
...
# ── [PATCH-B] 병렬 이미지 로드 + 세션 캐시 ─────────────────────────
def _fetch_one(args):
    """ThreadPoolExecutor용 단일 이미지 다운로드"""
    set_name, img_id_or_name = args
    return set_name, get_image_from_drive(img_id_or_name)
...
def preload_set_images_parallel(sets_dict: dict) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        results = list(executor.map(_fetch_one, tasks))
...
def create_advanced_pdf(final_data_list, service_items, quote_name, quote_date, form_type, price_labels, buyer_info, remarks):
    ...
    if form_type == "basic":
        COL_IMG  = 25; COL_INFO = 63; COL_UNIT = 13; COL_QTY  = 13
        COL_P1   = 29; COL_AMT  = 32; COL_RMK  = 15
    else:
        COL_IMG  = 25; COL_INFO = 55; COL_UNIT = 10; COL_QTY  = 10
        COL_P1   = 18; COL_AMT1 = 22; COL_P2   = 18; COL_AMT2 = 22; COL_PROF = 10
    ...
    for item in final_data_list:
        ...
        if img_b64:
            try:
                ...
                MARGIN = 4
                cell_w_px = int(14 * 7.5) - MARGIN * 2
                cell_h_px = int(ROW_H_ITEM * 4 / 3) - MARGIN * 2
        ...
        if form_type == "basic":
            ...
            a2 = p2 * qty; profit = a2 - a1; rate = (profit / a2 * 100) if a2 else 0
            total_a2 += a2
            ws.write(data_row, COL_QTY,  qty,            f_center_14_shrink)
            ws.write(data_row, COL_P1,   p1,             f_num_14_shrink)
...
def create_composition_pdf(set_cart, pipe_cart, final_data_list, db_products, db_sets, quote_name):
    ...
    header_h = 8
    col_w_img  = 40; col_w_name = 105; col_w_type = 25; col_w_qty  = 20
    ...
    if additional_items_list:
        ...
        for item in additional_items_list:
            check_page_break(16)
            name = item['name']; spec = item['spec'] if item['spec'] else '-'
            qty = item['qty']; code = item.get('code'); img_val = item.get('image')
            img_id = get_best_image_id(code, img_val, drive_file_map)
            img_b64 = download_image_by_id(img_id)
    ...
    for item in final_data_list:
        ...
        if qty == 0: continue
        check_page_break(16)
        name = item.get("품목", ""); spec = item.get("규격", "-")
        code = item.get("코드", ""); img_val = item.get("image_data")
        img_id = get_best_image_id(code, img_val, drive_file_map)
        img_b64 = download_image_by_id(img_id)
...
def create_composition_excel(set_cart, pipe_cart, final_data_list, db_products, db_sets, quote_name):
    ...
    for item in final_data_list:
        ...
        if code and code in temp_baseline:
            base_qty = temp_baseline[code]
            if total_qty > base_qty:
                additional_items_list.append({"name": name, "spec": spec, "qty": total_qty - base_qty, "code": code, "image": img_data})
                temp_baseline[code] = total_qty
            else:
                temp_baseline[code] -= total_qty
        else:
    ...
    def insert_scaled_image(ws, row, col, img_b64):
        if not img_b64:
            ws.write(row, col, "", fmt_center); return
        try:
            ...
            scale = min(cell_w_px / orig_w, cell_h_px / orig_h) * 0.9
            final_w = orig_w * scale; final_h = orig_h * scale
            ws.insert_image(row, col, tmp_path, {
                'x_scale': scale, 'y_scale': scale,
                'x_offset': (cell_w_px - final_w) / 2, 'y_offset': (cell_h_px - final_h) / 2,
                'object_position': 1
            })
        except:
    ...
    if additional_items_list:
        ...
        for item in additional_items_list:
            ...
            insert_scaled_image(ws_add, row, 0, download_image_by_id(get_best_image_id(item.get('code'), item.get('image'), drive_file_map)))
    ...
    for item in final_data_list:
        ...
        ws3.write(row, 2, item.get("규격", "-"), fmt_center); ws3.write(row, 3, qty, fmt_center); row += 1
...
if "_img_cache" not in st.session_state:   # [PATCH-E] 세션 이미지 캐시 초기화
    st.session_state._img_cache = {}

# ── [PATCH-E] 세트 카드 툴팁 CSS (전역, 한 번만) ─────────────────
st.markdown("""
<style>
.set-card-wrap {
    position: relative;
...
if mode == "관리자 모드" or mode == "管理者モード":
    ...
    if st.button("🔄 구글시트 데이터 새로고침"):
        st.session_state.db = load_data_from_sheet()
        st.session_state._img_cache = {}   # 이미지 캐시도 함께 초기화
    ...
    if not st.session_state.auth_admin:
                        ...
                        if not file_map:
                            ...
                            for p in products:
                                ...
                                if code and code in file_map:
                                    p["image"] = file_map[code]; updated_count += 1
                            if updated_count > 0:
                    ...
                    if new_buy_input > 0:
                        ...
                        with col_preview:
                            ...
                            for fk, label in KR_PRICE_LABELS.items():
                                ...
                                fmt_fn = lambda x: round(x, 1) if x < 1000 else int(x)
                                preview_rows.append({"_field": fk, "항목": label, "기존": fmt_fn(old_v), "변경후": fmt_fn(new_v)})
                        ...
                        if new_buy_input != old_buy:
                            ...
                            st.info("ℹ️ 매입가 동일 — 변경후 열을 직접 수정한 항목만 저장됩니다.")
        ...
        with t2:
            ...
            # ── [PATCH-D] 세트 이미지 빌더 ───────────────────────────
            with st.expander("🎨 세트 이미지 빌더 — 캔버스에서 직접 제작 후 드라이브 저장", expanded=False):
                st.caption(
                    "레시피가 등록된 세트의 이미지를 캔버스에서 만들고 구글드라이브에 바로 저장합니다.\n"
                    "기존처럼 외부에서 만든 이미지를 드라이브에 올린 뒤 아래 동기화 버튼으로 연결해도 됩니다."
                )
                ...
                if not builder_set_names:
                    ...
                    # ── 배경 모드 ────────────────────────────────────
                    with col_r2:
                        bg_mode = st.radio("배경 모드", ["정면도 (흰 배경)", "등사각 격자"], horizontal=True, key="builder_bg")

                    # ── 배관 도구 ────────────────────────────────────
                    st.markdown("**🖊️ 배관 표현 도구** (이미지에만 표현, 세트 데이터에 미포함)")
                    ...
                    pipe_color_js = "#FFD700" if "호스" in pipe_tool else "#555555"

                    # ── 부속 팔레트 ──────────────────────────────────
                    prod_code_to_name_builder = {
                        str(p.get("code", "")).strip().zfill(5): p.get("name", "")
                        for p in st.session_state.db.get("products", [])
                    }
                    if target_recipe:
                        st.markdown("**📦 레시피 구성품** (참고용 — 드래그 배치는 외부 이미지로)")
                        palette_cols = st.columns(min(len(target_recipe), 5))
                        fmap_builder = get_drive_file_map()
                        for idx, (code, qty) in enumerate(target_recipe.items()):
                            with palette_cols[idx % 5]:
                                p_name = prod_code_to_name_builder.get(str(code).zfill(5), code)
                                img_id = fmap_builder.get(str(code).zfill(5))
                                if img_id:
                                    b64 = get_cached_set_image(f"_part_{code}", img_id)
                                    if b64:
            ...
            ctx.lineWidth = 1;
            var step = 40;
            for(var xi = -canvas_h_val; xi < canvas_w_val + canvas_h_val; xi += step) {{
                ctx.beginPath(); ctx.moveTo(xi, 0); ctx.lineTo(xi + canvas_h_val, canvas_h_val); ctx.stroke();
                ctx.beginPath(); ctx.moveTo(xi, 0); ctx.lineTo(xi - canvas_h_val, canvas_h_val); ctx.stroke();
            }}
            ctx.restore();
"""
                    safe_target = builder_target.replace("'", "\\'").replace('"', '\\"')
                    pipe_opacity_val = pipe_opacity / 100.0

                    canvas_html = f"""
<style>
#builderCanvas {{ cursor: crosshair; border: 2px solid #ccc; border-radius: 6px; background: white; }}
#bToolbar {{ margin-bottom:8px; display:flex; gap:6px; flex-wrap:wrap; align-items:center; }}
#bToolbar button {{
    padding:5px 12px; border-radius:4px; border:1px solid #ccc;
    cursor:pointer; font-size:13px; background:#f8f8f8;
}}
#bToolbar button.active {{ background:#4a90d9; color:white; border-color:#4a90d9; }}
#bStatus {{ font-size:12px; color:#888; margin-left:4px; }}
#saveMsg {{ margin-top:6px; font-size:13px; color:green; display:none; }}
</style>
<div id="bToolbar">
  ...
  <span id="bStatus">모드: 선택</span>
</div>
<canvas id="builderCanvas" width="{canvas_w}" height="{canvas_h}"
        style="max-width:100%;"></canvas>
<br>
<button onclick="exportPNG()" style="margin-top:8px;padding:6px 18px;
        background:#27ae60;color:white;border:none;border-radius:5px;
        cursor:pointer;font-size:14px;">
  💾 PNG 내보내기 (파일 저장 후 아래 업로더로 드라이브 등록)
</button>
<a id="dlLink" style="display:none"></a>
<div id="saveMsg">✅ PNG 다운로드 완료 — 아래 업로더로 등록하세요.</div>

<script>
const canvas = document.getElementById('builderCanvas');
const ctx = canvas.getContext('2d');
const canvas_w_val = {canvas_w};
const canvas_h_val = {canvas_h};
let mode = 'select';
let drawing = false, startX, startY;
let history = [], pipes = [];
const pipeColor = '{pipe_color_js}';
const pipeWidth = {pipe_thickness};
const pipeOpacity = {pipe_opacity_val:.2f};

function setMode(m) {{
    mode = m;
    document.querySelectorAll('#bToolbar button').forEach(b => b.classList.remove('active'));
    const btnId = 'btn' + m.charAt(0).toUpperCase() + m.slice(1);
    if(document.getElementById(btnId)) document.getElementById(btnId).classList.add('active');
    document.getElementById('bStatus').textContent = '모드: ' + (m === 'pipe' ? '배관 그리기' : '선택');
    canvas.style.cursor = m === 'pipe' ? 'crosshair' : 'default';
}}

function redraw() {{
    ctx.clearRect(0,0,canvas_w_val,canvas_h_val);
    ctx.fillStyle = 'white';
    ctx.fillRect(0,0,canvas_w_val,canvas_h_val);
    {iso_grid_js}
    pipes.forEach(p => {{
        ctx.save();
...
canvas.addEventListener('mousedown', e => {{
    ...
    startX = (e.clientX - r.left) * (canvas_w_val / r.width);
    startY = (e.clientY - r.top) * (canvas_h_val / r.height);
    drawing = true;
}});
canvas.addEventListener('mouseup', e => {{
    if(mode !== 'pipe' || !drawing) return;
    const r = canvas.getBoundingClientRect();
    const endX = (e.clientX - r.left) * (canvas_w_val / r.width);
    const endY = (e.clientY - r.top) * (canvas_h_val / r.height);
    drawing = false;
    history.push(JSON.parse(JSON.stringify(pipes)));
    pipes.push({{x1:startX,y1:startY,x2:endX,y2:endY,color:pipeColor,width:pipeWidth,opacity:pipeOpacity}});
    redraw();
}});
...
function exportPNG() {{
    ...
    link.download = '{safe_target}.png';
    link.click();
    document.getElementById('saveMsg').style.display='block';
}}
</script>
"""
                    st.components.v1.html(canvas_html, height=canvas_h + 140, scrolling=False)

                    st.divider()
                    st.markdown("**📤 완성된 이미지를 드라이브에 등록**")
                    st.caption(
                        f"① PNG 내보내기로 저장 → ② 아래 업로더로 파일 선택 → ③ [드라이브 저장 및 세트 등록] 클릭\n"
                        f"또는 외부에서 만든 이미지를 **'{builder_target}.png'** 라는 이름으로 드라이브에 직접 올린 뒤 아래 [동기화] 버튼 사용"
                    )
                    builder_upload = st.file_uploader(
                        f"세트 이미지 업로드 ({builder_target})",
                        type=["png", "jpg", "jpeg"],
                        key=f"builder_uploader_{builder_target}"
                    )
                    if builder_upload:
                        if st.button("☁️ 드라이브 저장 및 세트 이미지 등록", key="btn_builder_save", type="primary"):
                            with st.spinner("구글드라이브에 업로드 중..."):
                                file_ext = builder_upload.name.split('.')[-1]
                                # 파일명 = 세트명.확장자 (기존 규칙과 동일)
                                drive_filename = f"{builder_target}.{file_ext}"
                                new_id = upload_bytes_to_drive(
                                    builder_upload.getvalue(), drive_filename,
                                    mimetype=builder_upload.type
                                )
                            if new_id:
                                st.session_state.db["sets"][builder_cat][builder_target]["image"] = new_id
                                save_sets_to_sheet(st.session_state.db["sets"])
                                # 캐시 무효화
                                st.session_state._img_cache.pop(builder_target, None)
                                get_drive_file_map.clear()
                                st.success(f"✅ '{builder_target}' 세트 이미지가 드라이브에 저장되고 등록되었습니다!")
                                st.rerun()
                            else:
                                st.error("드라이브 저장에 실패했습니다. 권한 또는 용량을 확인해주세요.")
            # ── [PATCH-D] 끝 ─────────────────────────────────────────
                            ...
                            if del_pw == admin_pwd_db:
                                ...
                                for name in target_names:
                                    if name in st.session_state.db["sets"][cat]:
                                        del st.session_state.db["sets"][cat][name]; del_count += 1
                                save_sets_to_sheet(st.session_state.db["sets"])
                    ...
                    with st.spinner("드라이브 폴더를 검색하는 중..."):
                        ...
                        if not file_map:
                            ...
                            if updated_count > 0:
                                ...
                                st.session_state._img_cache = {}   # 전체 캐시 초기화
            ...
            if mt == "신규":
                ...
                if st.session_state.temp_set_recipe:
                    recipe_list = [{"품목코드": str(k), "품목명": code_name_map.get(str(k), str(k)), "수량": int(v), "삭제": False} for k, v in st.session_state.temp_set_recipe.items()]
                    edited_recipe = st.data_editor(pd.DataFrame(recipe_list), num_rows="dynamic", width="stretch", hide_index=True, disabled=["품목코드", "품목명"], column_config={"삭제": st.column_config.CheckboxColumn(label="삭제?", default=False)}, key="recipe_editor_new")
                    new_recipe = {}
                ...
                if "target_set_edit" in st.session_state and st.session_state.target_set_edit:
                    ...
                    if st.session_state.temp_set_recipe:
                        recipe_list = [{"품목코드": str(k), "품목명": code_name_map.get(str(k), str(k)), "수량": int(v), "삭제": False} for k, v in st.session_state.temp_set_recipe.items()]
                        edited_recipe = st.data_editor(pd.DataFrame(recipe_list), num_rows="dynamic", width="stretch", hide_index=True, disabled=["품목코드", "품목명"], column_config={"삭제": st.column_config.CheckboxColumn(label="삭제?", default=False)}, key="recipe_editor_edit")
                        new_recipe = {}
                        for _, row in edited_recipe.iterrows():
                            if row.get("삭제"): continue
                            c = str(row.get("품목코드", "")).strip()
                    ...
                    if st.button("수정 내용 저장", type="primary"):
                        ...
                        save_sets_to_sheet(st.session_state.db["sets"]); st.success("수정되었습니다.")
                    st.write("")
...
# ==========================================
    ...
    if not jp_quotes:
        ...
        selected_quote_idx = st.selectbox("분석 대상 견적 선택", range(len(df_quotes)), format_func=lambda i: f"[{df_quotes.iloc[i].get('날짜','')}] {df_quotes.iloc[i].get('현장명','')}")
        ...
        try:
            ...
            items_dict = {}; st.error("JSON 데이터 파싱 실패")
        if items_dict:
            ...
            if st.button("📄 수출 이익 분석서 생성"):
                with st.spinner("보고서를 생성하고 있습니다..."):
                    ...
                    pdf = PDF(orientation='L'); pdf.title_text = "輸出利益分析書 (Export Profit Analysis)"; pdf.add_page()
                    ...
                    pdf.cell(0, 10, f"Quote Name: {target_quote.get('현장명')}", ln=True); pdf.ln(5)
                    pdf.set_fill_color(220, 220, 220)
                    cols = ["Code", "Item Name", "Spec", "Qty", "Buy Price", "Supply Price", "Sum Revenue", "Sum Cost", "Profit"]
                    widths = [20, 50, 40, 15, 30, 30, 35, 35, 30]
                    for head, w in zip(cols, widths): pdf.cell(w, 10, head, border=1, align='C', fill=True)
                    pdf.ln()
                    ...
                    for _, row in df_analysis.iterrows():
                        ...
                        pdf.cell(widths[8], 8, f"{int(row['순이익']):,}", border=1, align='R'); pdf.ln()
        ...
        if not jp_products:
            ...
            if st.session_state.quote_step == 1:
                ...
                with st.expander("セット選択", True):
                    ...
                    # ── [PATCH-C JP버전] render_inputs_jp — 캐시+툴팁 ─────
                    def render_inputs_jp(d, pf):
                        preload_set_images_parallel(d)
                        prod_code_to_name_jp = {
                            str(p.get("code", "")).strip().zfill(5): p.get("name", "")
                            for p in st.session_state.db.get("products", [])
                        }
                            ...
                            with cols[i % 4]:
                                ...
                                if recipe:
                                    tooltip_lines = "<br>".join([
                                        f"· {prod_code_to_name_jp.get(str(k).zfill(5), str(k))} &nbsp;×{qty}"
  ...
  <div class="set-card-tooltip">{tooltip_lines}</div>
    ...
    if st.session_state.quote_step == 1:
        ...
        with st.expander("1. 주배관 및 가지관 세트 선택", True):
            ...
            # ── [PATCH-C] render_inputs_with_key — 캐시+툴팁 버전 ────
...
<div class="set-card-wrap">
  ...
  <div class="set-card-tooltip">{tooltip_lines}</div>
            ...
            with c_btn1:
                if st.button("💾 세트 목록 변경사항 적용", use_container_width=True):
                    ...
                    for _, row in edited_cart.iterrows():
                        if not row.get("삭제"):
                            new_cart.append({"name": row["name"], "qty": int(row["qty"]), "type": row["type"]})
        ...
        key_map = {
            ...
            "계통농협":("price_nh_sys","계통"), "지역농협":("price_nh_loc","지역"), "단가(현장)":("price_site","현장")
        }
        rows = []
        pdb = {}
        for p in st.session_state.db["products"]:
        ...
        for n, q in st.session_state.quote_items.items():
            ...
            if view != "소비자가":
                k, l = key_map[view]; pr = inf.get(k, 0)
        ...
        if view == "소비자가": disp += ["소비자가", "합계"]
            ...
            l = key_map[view][1]; disp += [f"{l}단가", f"{l}합계", "소비자가", "합계", "이익", "율(%)"]
        if rows: df = pd.DataFrame(rows)
        else: df = pd.DataFrame(columns=disp)
        ...
        with st.expander("🛒 추가된 부품 수정 및 삭제", expanded=False):
            ...
            for k, v in st.session_state.quote_items.items():
                inf = pdb.get(str(k), {})
                parts_list.append({"품목코드": inf.get("code", str(k)), "품목명": inf.get("name", str(k)), "수량": int(v), "삭제": False, "_orig_key": str(k)})
            if parts_list:
        ...
        if not st.session_state.step3_ready or selectors_changed:
            ...
            st.session_state.final_edit_df = pd.DataFrame(fdata)
            st.session_state.step3_ready = True; st.session_state.last_sel = sel; st.session_state.files_ready = False
        ...
        edited = st.data_editor(
            ...
            column_config={
                "품목": st.column_config.TextColumn(required=True), "규격": st.column_config.TextColumn(),
                "코드": st.column_config.TextColumn(), "단위": st.column_config.TextColumn(),
        ...
        with c2:
            if st.button("🔄 처음으로"):
                st.session_state.quote_step = 1; st.session_state.quote_items = {}
                st.session_state.services = []; st.session_state.pipe_cart = []; st.session_state.set_cart = []
