...
"""🏪 아쿠나리스 빌더 — 전용 모듈 패키지. [2026-09-11] 루퍼젯 프로매니저에서 분리.
  ...
  layout.py      배치 엔진(표준 상수·패킹·자동배치·SVG)   ← 옛 루트 aquanaris_layout.py (V66~ · 상표 철자 오류 — 2026-09-11 폐기)
...
📌 배포 단위 = aqunaris_app.py + aqunaris/ + common/ (같은 저장소 Looperget-Mate/Price).
📌 모듈을 추가/변경하면 AQN_VER 를 올리고 aqunaris_app.py 가드 기준도 함께 올린다.
🔴 looperget(프로매니저 · 설계 개념)을 import 하지 않는다 — 대표 지시 · 시험으로 고정.
🔴 영문 표기 = **Aqunaris**(등록상표) — 파일·패키지·화면 모두 이 철자(2026-09-11 대표 정정 · 시험으로 고정).
"""
