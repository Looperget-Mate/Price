...
def design(site: Dict, price_db: Optional[Dict] = None, tier: str = "소비자가") -> Dict:
    ...
    #    펌프가 [미확정]이면 자동 선정이 순환이라 승인본 기본(호칭 50)으로 두고 그 사실을 말한다.
    from . import hydro_zone as _hz            # 지연 import — 물리 계산은 여기서만 쓴다
    _pick = _hz.resolve_main_mm(
    ...
    return {
        ...
        "main_mm": _pick["main_mm"], "main_mm_source": _pick["source"],
