...
def draft_sheet(fr, parcels=None, blocks_px=None, basemap="PHOTO_HYBRID", title=""):
    """농민 확인용 작도판 PNG — 위성 + 지적 참고선 + 지번 + 우리가 그린 대상지 + 축척 + 방위.

    `blocks_px` = {이름: [[x_px, y_px], ...]} — 같은 화소를 `blocks_from_pixels` 가 그대로 받는다.
    지번을 크게 적는 이유 = 농민이 「우리 밭 211-1」이라고 말할 수 있어야 하기 때문이다.
    """
    import io as _io
    from PIL import Image, ImageDraw
    im = Image.open(_io.BytesIO(print_tile(fr, basemap=basemap))).convert("RGB")
...
def draft_from_address(query, zoom=18, size=1024, radius_m=150.0, blocks_px=None):
    ...
    png = draft_sheet(fr, parcels=near, blocks_px=blocks_px,
